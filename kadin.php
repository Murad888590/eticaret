<?php
   ...
   $toplamKayitSayisiSorgusu = $db->prepare("SELECT * FROM goods WHERE durumu = 1 AND urunTuru = 'kadin'  $menuKosulu $aramaKosulu ORDER BY id DESC");
   ...
   $menuCountFetch = $db->prepare("SELECT SUM(urunSayisi) AS menununToplamUrunu FROM menuler WHERE urunTuru = 'kadin'");
...
<div class="manWrapper">
   ...
   <div class="manWrapper__items">
      ...
      <div class="manWrapper__items__wrapper">
         <?php
            $goodsFetch = $db->prepare("SELECT * FROM goods WHERE durumu = 1 AND urunTuru = 'kadin' $menuKosulu $aramaKosulu ORDER BY id DESC LIMIT $sayfalamayBaslayacaqKayotSayisi, $sayfaBasinaGosterilecek");
            $goodsFetch->execute();
                  ...
                  foreach($goods as $good) {
                     ...
                     <div class="manWrapper__item">
                        <div class="manWrapper__item__img">
                          <a href="qadin-ayakkabisi/<?=donusumleriGeriDondur($good["urun_adi"])?>/<?=donusumleriGeriDondur($good["id"])?>"><img style="height: 250px;" src="assets/images/UrunResimleri/Kadin/<?=donusumleriGeriDondur($good["urun_resmi_bir"])?>" alt=""></a>
                        </div>
         ...
         if($bulunanSafyaSayisi>1) {?>
            <div class="paginationWrapper">
               <nav aria-label="Page navigation example ">
                  <ul class="pagination">
                  <li class="page-item"><a class="page-link" href="qadinayakkabisayfala/1/<?$sayfalamaKosulu?>">&laquo;</a></li>
                  <?php
                     for($i = $sayfalama-$sayfalamaIcinButonSayisi; $i <= $sayfalama+$sayfalamaIcinButonSayisi; $i++) {
                        ...
                        if($sayfalama == $i) {
                           ...
                           echo "<li class=\"page-item\"><a class=\"page-link\" href=\"qadinayakkabisayfala/$curr$sayfalamaKosulu\">$curr</a></li>";
                        }
                     }
                  }
                  ?>
                     
                     <li class="page-item"><a class="page-link"  href="qadinayakkabisayfala/<?=$bulunanSafyaSayisi?><?$sayfalamaKosulu?>">&raquo;</a></li>
                  </ul>
               </nav>
            </div>
         <?php
